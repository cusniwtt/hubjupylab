...
def test_gpu_features():
    ...
    # Backup original gpu_config
    old_config = db.get_gpu_config()
    ...
    try:
        ...
        db.save_gpu_config(
            ssh_host=old_config.get('ssh_host', ''),
            ssh_port=old_config.get('ssh_port', 22),
            ssh_user=old_config.get('ssh_user', 'root'),
            ssh_key_path=old_config.get('ssh_key_path', ''),
            remote_base_dir=old_config.get('remote_base_dir', '/workspace')
        )
        print("Restored original GPU config.")
